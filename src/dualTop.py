...
def change_names(thirdb):
	...
	for i in range(len(thirdb)):
		...
		if len(t)<3:
			...
			c=t[2]
		print a,b,c 
		if (b.isalpha()) == False or (c.isalpha()) == False:
			if (c.isalpha()) == True:
				c=c+1
...
with open('reference.rtf','r') as infile1:   ######ALLAZEIS EDWWW
	with open('smutant.rtf','r') as infile2:             ######ALLAZEIS EDWWW auto pou exeis parei apo to merge2.py
